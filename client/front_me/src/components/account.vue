    ...
    <div v-show="!isInfoEdit && !isPasswordEdit">
      ...
      <div class="weui-cells weui-cells_form">
        ...
        <div class="weui-cell">
          ...
          <div class="weui-cell__bd">¥{{ user.wallet || 0 }}</div>
        </div>
      </div>
      ...
      <div class="weui-cells">
        <a class="weui-cell weui-cell_access" v-for="materialBook in materialBookList" @click="materialBookUpdateFail(materialBook._id)">
          <div class="weui-cell__bd">
            活动：<span class="highlight">{{ materialBook.activity }}</span><br>
            预约人：{{ materialBook.name }} (<a :href="'tel:' + materialBook.phone">{{ materialBook.phone }}</a>)<br>
            领取时间：{{ materialBook.takeDate }}<br>
            归还时间：<span class="highlight">{{ materialBook.returnDate }}</span><br>
            借用物资：<label v-for="(book, index) of materialBook.book"><b>({{ index + 1 }})&nbsp{{ book.name }}{{ book.book }}{{ book.unit }}&nbsp&nbsp</b></label><br>
            物资总价：<span class="highlight">共 ¥{{ materialBook.sum || 0 }}</span>&nbsp&nbsp&nbsp&nbsp目前状态：<span class="highlight">{{ materialBook.condition }}</span>
          </div>
          <div class="weui-cell__ft"></div>
        </a>
      ...
      <div class="weui-cells">
        <a class="weui-cell weui-cell_access" v-for="meetingBook in meetingBookList" @click="meetingBookUpdateFail(meetingBook._id)">
          <div class="weui-cell__bd">
            会议：<span class="highlight">{{ meetingBook.activity }}</span><br>
            预约人：{{ meetingBook.name }} (<a :href="'tel:' + meetingBook.phone">{{ meetingBook.phone }}</a>)<br>
            预约日期：{{ meetingBook.date }}<br>
            预约时间：<span class="highlight">{{ meetingBook.time }}</span>
          </div>
          <div class="weui-cell__ft"></div>
        </a>
